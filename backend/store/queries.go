...
// GetEnrolByUser will return a list of enrollments for a specific user
func (store *Store) GetEnrolByUser(uid string) ([]Enrol, error) {
	query := "SELECT course_id, user_uid, role FROM enrol WHERE user_uid = $1"
	...
	for rows.Next() {
		var courseID int
		var userUID, role string
		err := rows.Scan(&courseID, &userUID, &role)
		if err != nil {
			log.Println(err.Error())
			log.Println("Failed in rows.Next for GetEnrolByUser")
			continue
		}
		enrol = append(enrol, Enrol{
			CourseID: courseID,
			UserUID:  userUID,
			Role:     role,
		})
